...
** GrammarMatcher's job and is tested separately.
*/
...
/* AbnfCompiler builds, Grammar holds. This pairs them so a test can say
...
class Compiled {
 public:
  ...
  const GrammarNode& rootOf(const char* name) const {
...
TEST(AbnfCompilerTest, LoadsRfc2812Section231) {
  Compiled c;
...
TEST(AbnfCompilerTest, IndentedNameElidedIncrementalAlternativeBecomesAnAlt) {
  Compiled c;
  ASSERT_TRUE(c.from(kRfc2812)) << c.error;

  ASSERT_GE(c.rule("params"), 0);
  EXPECT_EQ(c.rootOf("params").kind, GrammarNode::Alternation);
  EXPECT_EQ(c.rootOf("params").count, 2);
}

/* `chanstring =/ ...` at column 0 is the ordinary spelling and must work too.
 */
TEST(AbnfCompilerTest, ColumnZeroIncrementalAlternativeBecomesAnAlt) {
  Compiled c;
  ASSERT_TRUE(c.from(kRfc2812)) << c.error;

  ASSERT_GE(c.rule("chanstring"), 0);
  EXPECT_EQ(c.rootOf("chanstring").kind, GrammarNode::Alternation);
}

/* ── Individual constructs ── */

TEST(AbnfCompilerTest, OctetRange) {
  Compiled c;
  ASSERT_TRUE(c.from("a = %x41-5A\n")) << c.error;
  const GrammarNode& n = c.rootOf("a");
  EXPECT_EQ(n.kind, GrammarNode::OctetRange);
  EXPECT_EQ(n.lo, 0x41);
  EXPECT_EQ(n.hi, 0x5A);
}

TEST(AbnfCompilerTest, SingleOctetIsADegenerateRange) {
  Compiled c;
...
TEST(AbnfCompilerTest, DottedOctetConcatenation) {
  Compiled c;
  ASSERT_TRUE(c.from("a = %x0D.0A\n")) << c.error;
  const GrammarNode& n = c.rootOf("a");
  EXPECT_EQ(n.kind, GrammarNode::Sequence);
  EXPECT_EQ(n.count, 2);
}

TEST(AbnfCompilerTest, Literal) {
  Compiled c;
  ASSERT_TRUE(c.from("a = \"PASS\"\n")) << c.error;
  const GrammarNode& n = c.rootOf("a");
  ASSERT_EQ(n.kind, GrammarNode::Literal);
  EXPECT_EQ(c.grammar.literal(n.literal), "PASS");
}

TEST(AbnfCompilerTest, Alternation) {
  Compiled c;
  ASSERT_TRUE(c.from("a = \"x\" / \"y\" / \"z\"\n")) << c.error;
  const GrammarNode& n = c.rootOf("a");
  EXPECT_EQ(n.kind, GrammarNode::Alternation);
  EXPECT_EQ(n.count, 3);
}

TEST(AbnfCompilerTest, Concatenation) {
  Compiled c;
...
TEST(AbnfCompilerTest, StarRepetitionIsUnbounded) {
  Compiled c;
  ASSERT_TRUE(c.from("a = *\"x\"\n")) << c.error;
  const GrammarNode& n = c.rootOf("a");
  ASSERT_EQ(n.kind, GrammarNode::Repetition);
  EXPECT_EQ(n.lo, 0);
  EXPECT_EQ(n.hi, GrammarNode::kUnbounded);
}

TEST(AbnfCompilerTest, BoundedRepetitionCarriesBothBounds) {
  Compiled c;
...
TEST(AbnfCompilerTest, ExactRepetition) {
  Compiled c;
...
TEST(AbnfCompilerTest, UpperBoundOnlyRepetition) {
  Compiled c;
...
TEST(AbnfCompilerTest, OptionIsAZeroOrOneRepetition) {
  Compiled c;
  ASSERT_TRUE(c.from("a = [ \"x\" ]\n")) << c.error;
  const GrammarNode& n = c.rootOf("a");
  ASSERT_EQ(n.kind, GrammarNode::Repetition);
...
TEST(AbnfCompilerTest, CommentsAndBlankLinesAreIgnored) {
  Compiled c;
  ASSERT_TRUE(
...
TEST(AbnfCompilerTest, SemicolonInsideALiteralIsNotAComment) {
  Compiled c;
...
TEST(AbnfCompilerTest, IndentedLineFoldsIntoThePreviousRule) {
  Compiled c;
  ASSERT_TRUE(
      c.from("a = \"x\"\n"
             "      \"y\"\n"))
...
TEST(AbnfCompilerTest, RuleNamesAreCaseInsensitive) {
  Compiled c;
...
TEST(AbnfCompilerTest, DollarMarksACapture) {
  Compiled c;
  ASSERT_TRUE(
      c.from("b = \"y\"\n"
             "a = \"x\" $b\n"))
...
TEST(AbnfCompilerTest, UncapturedReferenceRecordsNothing) {
  Compiled c;
  ASSERT_TRUE(c.from("b = \"y\"\na = b\n")) << c.error;
  EXPECT_EQ(c.grammar.captureCount(), 0u);
}

TEST(AbnfCompilerTest, RepeatedCaptureNameInternsOnce) {
  Compiled c;
  ASSERT_TRUE(
      c.from("b = \"y\"\n"
             "a = $b $b\n"))
...
TEST(AbnfCompilerTest, UserCommandProductionCapturesItsFourFields) {
...
TEST(AbnfCompilerTest, AcceptsRightRecursion) {
  Compiled c;
  EXPECT_TRUE(c.from("a = \"x\" [ a ]\n")) << c.error;
}

/* A nullable leading element still leaves the rule at its own left edge. */
TEST(AbnfCompilerTest, RejectsLeftRecursionBehindANullablePrefix) {
  Compiled c;
  EXPECT_FALSE(c.from("a = [ \"x\" ] a\n"));
  EXPECT_NE(c.error.find("left-recursive"), std::string::npos) << c.error;
}

TEST(AbnfCompilerTest, RejectsIncrementalAlternativeBeforeDefinition) {
  Compiled c;
  EXPECT_FALSE(c.from("a =/ \"x\"\n"));
  EXPECT_NE(c.error.find("before that rule"), std::string::npos) << c.error;
}

TEST(AbnfCompilerTest, RejectsDuplicateDefinition) {
  Compiled c;
  EXPECT_FALSE(c.from("a = \"x\"\na = \"y\"\n"));
  EXPECT_NE(c.error.find("defined twice"), std::string::npos) << c.error;
}

TEST(AbnfCompilerTest, RejectsUnclosedGroup) {
  Compiled c;
  EXPECT_FALSE(c.from("a = ( \"x\"\n"));
}

TEST(AbnfCompilerTest, RejectsUnclosedOption) {
  Compiled c;
  EXPECT_FALSE(c.from("a = [ \"x\"\n"));
}

TEST(AbnfCompilerTest, RejectsUnterminatedLiteral) {
  Compiled c;
  EXPECT_FALSE(c.from("a = \"x\n"));
}

TEST(AbnfCompilerTest, RejectsBackwardsOctetRange) {
  Compiled c;
  EXPECT_FALSE(c.from("a = %x5A-41\n"));
}

TEST(AbnfCompilerTest, RejectsBackwardsRepetitionRange) {
  Compiled c;
  EXPECT_FALSE(c.from("a = 5*2\"x\"\n"));
}

TEST(AbnfCompilerTest, RejectsMissingDefinedAs) {
  Compiled c;
  EXPECT_FALSE(c.from("a \"x\"\n"));
}

TEST(AbnfCompilerTest, RejectsDollarWithoutARuleName) {
  Compiled c;
  EXPECT_FALSE(c.from("a = $\"x\"\n"));
}

TEST(AbnfCompilerTest, RejectsNonHexNumVal) {
  Compiled c;
  EXPECT_FALSE(c.from("a = %d65\n"));
}

/* The error must say *where*, or a typo in a 40-line grammar is a hunt. */
TEST(AbnfCompilerTest, ErrorNamesTheOffendingLine) {
  Compiled c;
  EXPECT_FALSE(
      c.from("a = \"x\"\n"
             "b = \"y\"\n"
