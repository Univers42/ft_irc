...
PlatformBus::~PlatformBus() {
  for (std::map<int, Conn>::iterator it = _conns.begin(); it != _conns.end();
       ++it)
    close(it->first);
...
void PlatformBus::onFdEvent(Server& server, int fd, uint32_t events) {
  ...
  if (events & EPOLLIN) handleInput(fd);
  if ((events & (EPOLLERR | EPOLLHUP)) && owns(fd)) closeConnection(fd);
}
...
bool PlatformBus::start() {
  ...
  Log::info("platform-bus listening on 127.0.0.1:" +
            libcpp::str::to_string(_port));
  return true;
}

bool PlatformBus::owns(int fd) const { return _conns.find(fd) != _conns.end(); }
...
void PlatformBus::closeConnection(int fd) {
  if (_conns.find(fd) == _conns.end()) return;
...
void PlatformBus::handleLine(int fd, const std::string& line) {
  ...
  std::string cmd = libcpp::str::to_upper(
